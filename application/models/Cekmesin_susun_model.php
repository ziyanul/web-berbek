...
class Cekmesin_susun_model extends CI_Model
{
    ...
    public function get_susun()
    {
        $susun = $this->susun;
        $this->db->select('p.uuid, v.varian, v.keterangan, p.tanggal');
        $this->db->select("(SELECT DATE_FORMAT(c.created_at, '%d %b %Y') FROM cek_mesin c WHERE p.uuid = c.t_planning_uuid AND c.area_uuid = '$susun' ORDER BY c.created_at ASC LIMIT 1) as awal_cek", false);
        $this->db->select("(SELECT count(DISTINCT c.mesin_uuid) FROM cek_mesin c WHERE c.t_planning_uuid = p.uuid AND c.area_uuid = '$susun') as jumlah_mesin", false);
