...
class Cekmesin_retort_model extends CI_Model
{
    ...
    {
        $retort = $this->retort;
        $this->db->select('p.uuid, v.varian, v.keterangan, p.tanggal, p.created_at');
        $this->db->select("(SELECT DATE_FORMAT(c.created_at, '%d %b %Y') FROM cek_mesin c WHERE p.uuid = c.t_planning_uuid AND c.area_uuid = '$retort' ORDER BY c.created_at ASC LIMIT 1) as awal_cek", false);
        $this->db->select("(SELECT count(DISTINCT c.mesin_uuid) FROM cek_mesin c WHERE c.t_planning_uuid = p.uuid AND c.area_uuid = '$retort') as jumlah_mesin", false);
    ...
    public function get_area_data($t_planning_uuid, $area_uuid)
    {
        ...
        $this->db->select(
            'c.*, c.uuid as cek_uuid, v.varian as varian_name, v.keterangan, t.tanggal, u.fullname, m.nama_mesin,
        ...
        $this->db->order_by('m.nama_mesin, c.item');
