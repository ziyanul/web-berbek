...
class TLmesin_model extends CI_Model
{
	...
	public function get_group_area_bulan() {
        $this->db->select('t.*,a.nama_area');
		$this->db->from('pengecekan_tools t');
		$this->db->join('area a', 'a.uuid=t.area_uuid', 'left');
		$this->db->order_by('t.created_at', 'DESC');
		$this->db->group_by('DATE_FORMAT(t.created_at, "%Y-%m"), t.area_uuid');
		$data = $this->db->get()->result();
		foreach ($data as $v) {
			$v->tgl = date('d M Y', strtotime($v->created_at));
			$v->bln = date('Y-m', strtotime($v->created_at));
        }
		return $data;
    }
  ...
  public function get_by_area_bulan($area_uuid, $bulan) {
    ...
    // Pastikan setiap tanggal memiliki semua UUID mesin
    foreach ($result as $tanggal => &$toolsData) {
        foreach ($uniqueTools as $uuid) {
            if (!isset($toolsData[$uuid])) {
                $toolsData[$uuid] = ['kondisi' => '-', 'kelengkapan' => '-', 'keterangan' => '-'];
            }
        }
    }

    // Kembalikan data dan area_uuid
    return [
        'data' => $result,
        'tools' => $uniqueTools,
        'area_uuid' => $area
