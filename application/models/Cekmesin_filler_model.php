...
class Cekmesin_filler_model extends CI_Model
{
    ...
    {
        $filler = $this->filler;
        $this->db->select('p.uuid, v.varian, p.tanggal');
        $this->db->select("(SELECT DATE_FORMAT(c.created_at, '%d %b %Y') FROM cek_mesin c WHERE p.uuid = c.t_planning_uuid AND c.area_uuid = '$filler' ORDER BY c.created_at ASC LIMIT 1) as awal_cek", false);
        $this->db->select("(SELECT count(DISTINCT c.mesin_uuid) FROM cek_mesin c WHERE c.t_planning_uuid = p.uuid AND c.area_uuid = '$filler') as jumlah_mesin", false);
        $this->db->from('t_planning p');
        $this->db->join('varian v', 'v.uuid = p.varian', 'left');
        $this->db->order_by('p.tanggal', 'DESC');
        $this->db->order_by('p.created_at', 'DESCd');
    ...
    {
        return $this->db->select('nama_area')->from('area')->where('uuid', $uuid)->get()->row();
    }
    ...
    public function get_area_data($t_planning_uuid, $area_uuid)
    {
        $area_uuid = $this->filler;
        $this->db->select(
            'c.*, c.uuid as cek_uuid, t.varian, m.nama_mesin, m.uuid as mesin_uuid, u.fullname,
        ...
        $this->db->order_by('m.nama_mesin DESC, c.id');
        ...
        foreach ($data as $val) {
            ...
            $mesin_headers[$val->mesin_uuid] = $mesin_headers[$val->mesin_uuid] ?? $val->nama_mesin;
        }

        return ['data' => $data, 'mesin_headers' => $mesin_headers];
    }


    public function get_by_uuid($t_planning_uuid, $area_uuid)
    {
        $this->db->select(
            't.uuid, v.varian, v.keterangan, t.tanggal, c.id, c.area_uuid, c.created_at, c.created_at as tgl_cek,
        ...
        $this->db->join('varian v', 'v.uuid = t.varian', 'left');
