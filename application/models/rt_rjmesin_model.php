...
class Rt_rjmesin_model extends CI_Model
{
	...
	{
		$this->db->select('pl.uuid as planning_uuid, v.varian as nama_varian, tb.kode_batch, pl.created_at');
		$this->db->from('t_planning pl');
		$this->db->join('varian v', 'pl.varian_uuid = v.uuid', 'left');
		$this->db->join('tbatch tb', 'pl.uuid = tb.t_planning_uuid', 'left');
	...
	{
		$this->db->select('tp.uuid as planning_uuid, m.created_at, tp.fr_rt_rjmesin, m.kode_batch, tp.tanggal, u.fullname as foreman_name, u1.fullname as spv_name, v.varian as nama_varian, v.keterangan');
		$this->db->from('tbatch m');
		$this->db->join('t_planning tp', 'tp.uuid = m.t_planning_uuid', 'left');
		...
		foreach ($data as $val) {
			...
			$val->batch_ke = substr($val->kode_batch, 5, 2);
			$val->tgl = date('d M Y', strtotime($val->tanggal));
		}
		return $data;
	}

	public function get_by_code($kode_batch)
	{
		$this->db->select('m.*, v.varian as nama_varian, v.keterangan');
		$this->db->from('tbatch m');
		$this->db->join('t_planning tp', 'tp.uuid = m.t_planning_uuid', 'left');
		$this->db->join('varian v', 'v.uuid = tp.varian', 'left');
		$this->db->where('m.kode_batch', $kode_batch);
		$this->db->order_by('m.created_at', 'DESC');
		$data = $this->db->get()->row();

		$data->tanggal = date('d M Y', strtotime($data->tanggal_produksi));
		return $data;
	}
	...
	public function get_mesin($kode_batch)
	{
		...
		$this->db->where('tb.kode_batch', $kode_batch);
		$this->db->group_by('tc.device_id');
		$data = $this->db->get()->result();
