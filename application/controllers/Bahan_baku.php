...
class Bahan_baku extends CI_Controller {
	...
	{
		$data = array(
			'data' => $this->bahan_baku_model->get_filler(),
			'active_nav' => 'bahan_filler'
		);

		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/bb-filler', $data);
		$this->load->view('partials/footer');
	}

	public function detailfiller($tanggal)
	{
		$data = array(
			'data' => $this->bahan_baku_model->get_all_filler($tanggal),
			'active_nav' => 'bahan_filler'
		);
		// echo "<pre>";
		// print_r ($data);
		// echo "</pre>";
		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/detail-filler', $data);
		$this->load->view('partials/footer');
	}

	public function detailpart($tanggal)
	{
		$data = array(
			'data' => $this->bahan_baku_model->get_all_sparepart($tanggal),
			'active_nav' => 'bahan_sparepart'
		);
		// echo "<pre>";
		// print_r ($data);
		// echo "</pre>";
		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/detail-part', $data);
		$this->load->view('partials/footer');
	}

	public function approval($tanggal,$area_uuid)
{
    $update = $this->bahan_baku_model->approval($tanggal,$area_uuid);
...
{
    $update = $this->bahan_baku_model->approval_filler($tanggal, $area_uuid);
    ...
    if ($update) {
        ...
        echo json_encode([
            'status' => true,
            'fullname' => $current_user->fullname, // pastikan nama diambil dari spv_uuid
            'message' => 'Reservasi berhasil di-ACC.',
        ]);
    } else {
        echo json_encode([
	...
	public function kirimfiller($bahan_uuid)
	{

		$data = array(
			'data' => $this->bahan_baku_model->get_kirim_filler($bahan_uuid),
			'nav' => $this->bahan_baku_model->get_by_uuid($bahan_uuid),
			'active_nav' => 'bahan_filler'
		);
		// echo "<pre>";
		// print_r ($data);
		// echo "</pre>";
		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/kirim-filler', $data);
		$this->load->view('partials/footer');
	}

	public function kirimpart($bahan_uuid)
	{

		$data = array(
			'data' => $this->bahan_baku_model->get_kirim_filler($bahan_uuid),
			'nav' => $this->bahan_baku_model->get_by_uuid2($bahan_uuid),
			'active_nav' => 'bahan_sparepart'
		);
		// echo "<pre>";
		// print_r ($data);
		// echo "</pre>";
		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/kirim-part', $data);
		$this->load->view('partials/footer');
	}

	public function diterima($uuid)
	{
		$update = $this->bahan_baku_model->update_diterima($uuid);
		if ($update) {
	...
	public function tambahfiller()
	{
		$rules = $this->bahan_baku_model->rules();
		$this->form_validation->set_rules($rules);
		
		if ($this->form_validation->run() === TRUE) {
		$today = date('Y-m-d');
		$tgl = date('Y-m-d', strtotime($today));
			$insert = $this->bahan_baku_model->insert_filler();
			if ($insert) {
				$this->session->set_flashdata('success_msg', 'Data berhasil di simpan.');
				redirect('bahan_filler/detail_'.$tgl);
			} else {
				redirect('bahan_filler/detail_'.$tgl);
		...
		$data = array(
			'jenis' => $this->bahan_baku_model->get_jenis_filler(),
			'urut' => $this->bahan_baku_model->generate_reservasi_number(),
			'active_nav' => 'bahan_filler'
		);
		...
		// echo "</pre>";
		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/tambah-filler');
		$this->load->view('partials/footer');
	}

	public function get_item_filler($uuid)
	{
		$data = $this->bahan_baku_model->get_item_filler($uuid);
		print_r(json_encode($data));
	}

	public function get_part_name($uuid)
	{
		$data = $this->bahan_baku_model->get_item_part($uuid);
		print_r(json_encode($data));
	}
	...
	{
		$data = array(
			'data' => $this->bahan_baku_model->get_sparepart(),
			'active_nav' => 'bahan_sparepart'
		);

		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/bb-part', $data);
		$this->load->view('partials/footer');
	}
	...
	public function tambahpart()
	{
		...
		$today = date('Y-m-d');
		$tgl = date('Y-m-d', strtotime($today));
			$insert = $this->bahan_baku_model->insert_part();
			if ($insert) {
				$this->session->set_flashdata('success_msg', 'Data berhasil di simpan.');
				redirect('bahan_sparepart/detail_'.$tgl);
			} else {
				redirect('bahan_sparepart/detail_'.$tgl);
				$this->session->set_flashdata('error_msg', 'Data gagal di simpan.');
			}
		...
		$data = array(
			...
			'urut' => $this->bahan_baku_model->reservasi_part(),
			'active_nav' => 'bahan_sparepart'
		);

		$this->load->view('partials/head', $data);
		$this->load->view('bahan-baku/tambah-part');
		$this->load->view('partials/footer');
	}

	public function formfiller($tanggal)
	{
		$options = new Options();
		$options->set('isRemoteEnabled', true);
		$options->set('defaultFont', 'DejaVu Sans');
		$dompdf = new Dompdf($options);
		$data = $this->bahan_baku_model->get_form_filler($tanggal);
		...
		<td style="border-left:0;border-right:0;border-bottom:0;height:28px;">&nbsp;2 dari 5</td> 
		</tr>
		</tbody>
		</table>
		</td>
		...
		<td style="border:none;"> : Filler</td>
...
public function formpart($tanggal)
{
$options = new Options();
$options->set('isRemoteEnabled', true);
$options->set('defaultFont', 'DejaVu Sans');
$dompdf = new Dompdf($options);
$data = $this->bahan_baku_model->get_form_part($tanggal);
$logo = base_url("assets/img/cpi-logo.jpg");
$html = '
<html>
...
<body>
        ...
        <tr>
            <td width="100">
                <table width="100%">
                    <tbody>
                        <tr>
                            <td rowspan="2" align="center" valign="middle" style="border:0;"><img src="'.$logo.'"
                                    width="100px"></td>
                        </tr>
                    </tbody>
                </table>
            ...
            <td width="650">
                <table width="100%">
                    <tbody>
                        <tr>
                            <td style="text-align:center;border-top:0;border-left:0;border-right:0;font-size: 12px;">
                                <h2>FORM</h2>
                            </td>
                        </tr>
                        <tr>
                            <td style="text-align:center;border:0;font-size: 12px;">
                                <h2>PENERIMAAN BAHAN BAKU AREA PRODUKSI</h2>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table width="101%" style="margin-left:-1px;">
                    <tbody>
                        ...
                        <tr>
                            <td style="border-left:0;border-right:0;border-bottom:0;height:28px;">&nbsp;Halaman</td>
                            <td style="border-left:0;border-right:0;border-bottom:0;height:28px;">:</td>
                            <td style="border-left:0;border-right:0;border-bottom:0;height:28px;">&nbsp;3 dari 5</td>
                        </tr>
                    </tbody>
                </table>
            </td>
    ...
    <table style="font-size: 12px;">
        <tbody>
            <tr>
                ...
                <td style="border:none;">:&nbsp;Sparepart</td>
                <td style="border:none;"></u></td>
                <td style="border:none;" width="700px"></td>
                <td style="border:none;">No. Reservasi</td>
                <td style="border:none;"> : '. sprintf("%04d", ($data[0]->no_reservasi)) .'</td>
            </tr>
            <tr>
                <td style="border:none;">Tanggal</td>
		...
		$no = 1;
            ...
            foreach ($itemGroups as $key => $group) {
                ...
                foreach ($group as $index => $row) {
                    ...
                    // Kolom lainnya tanpa merge
                    $html.= '<td>' . $row->qty_dikirim . '</td>';
                    $html.= '<td>' . $row->jam_kirim2 . '</td>';
                    $html.= '<td>' . $row->jam_terima2 . '</td>';
                }
            }
            $html .= '
        </tbody>
    ...
    <table style="font-size: 12px;" width="100%">
        <tbody>
            ...
            $html .= '<tr style="text-align:center;">';
                $html .= '<td height="50px;">' . $row->fullname . '</td>';
                $html .= '<td style="border:none;" width="15%"></td>';
                $html .= '<td>' . $row->pengirim . '</td>';
                $html .= '<td style="border:none;" width="15%"></td>';
                $html .= '<td>' . $row->spv . '</td>';
                $html .= '</tr>
