...
class Yieldportal extends CI_Controller
{
    ...
    {
        $s = $ss->getActiveSheet();
        $this->excel_title($s, 'RINGKASAN ANALISA YIELD', $scope, 'M');
        ...
        $r = 5;
        $d = $data ?: (object)array_fill_keys(['total_batch', 'adonan_formula', 'filkar_box', 'filkar_kg', 'sortasi_box', 'release_box', 'blm_sortir', 'yield_formula', 'yield_release', 'filkar_rework', 'filkar_reject', 'sortasi_rework', 'sortasi_reject'], 0);
        foreach (array_values((array)$d) as $i => $v) {
            if ($i > 12) break;
            $s->setCellValueByColumnAndRow($i + 1, $r, (float)$v);
        }
    ...
    {
        $s = $ss->createSheet();
        $this->excel_title($s, 'MONITORING PRODUKSI', $scope, 'M');
        ...
        foreach ($h as $i => $v) $s->setCellValueByColumnAndRow($i + 1, 4, $v);
        $this->excel_header($s, 'A4:M4');
        $r = 5;
        foreach ($data['rows'] as $d) {
            $vals = [$d->nama_varian, $d->adonan_formula, $d->filkar_box, $d->filkar_kg, $d->sortasi_box, $d->release_box, $d->blm_sortir, $d->filkar_rework, $d->filkar_reject, $d->sortasi_rework, $d->sortasi_reject, $d->yield_formula, $d->yield_release];
            foreach ($vals as $i => $v) $s->setCellValueByColumnAndRow($i + 1, $r, $v);
            $r++;
        }
        foreach (range('A', 'M') as $c) $s->getColumnDimension($c)->setAutoSize(true);
    }
    ...
    {
        $s = $ss->createSheet();
        $cols = 2 + count($data['varian']) + 1;
        $last = $this->excel_col($cols);
        $this->excel_title($s, 'BAD PRODUK PER VARIAN', $scope, $last);
        $h = ['Bad Produk', 'Proses'];
        foreach ($data['varian'] as $v) $h[] = $v->varian;
        $h[] = 'TOTAL';
        foreach ($h as $i => $v) $s->setCellValueByColumnAndRow($i + 1, 4, $v);
        $this->excel_header($s, "A4:{$last}4");
        $r = 5;
        foreach ($data['rows'] as $d) {
            $vals = [$d->nama_badpro, $d->proses];
            foreach ($data['varian'] as $v) $vals[] = (float)($d->{$v->uuid} ?? 0);
            $vals[] = (float)$d->total;
            foreach ($vals as $i => $v) $s->setCellValueByColumnAndRow($i + 1, $r, $v);
            $r++;
        }
        for ($i = 1; $i <= $cols; $i++) $s->getColumnDimension($this->excel_col($i))->setAutoSize(true);
    }
    private function excel_bad_mesin($ss, $data, $scope)
    {
        $s = $ss->createSheet();
        $cols = count($data['badproduk']) + 2;
        $last = $this->excel_col($cols);
        $this->excel_title($s, 'BAD PRODUK PER MESIN', $scope, $last);
        $h = ['Mesin'];
        foreach ($data['badproduk'] as $bp) $h[] = $bp->nama_badpro;
        $h[] = 'TOTAL';
        foreach ($h as $i => $v) $s->setCellValueByColumnAndRow($i + 1, 4, $v);
        $this->excel_header($s, "A4:{$last}4");
        $r = 5;
        foreach ($data['rows'] as $d) {
            $vals = [$d->mesin];
            foreach ($data['badproduk'] as $bp) $vals[] = (float)($d->{$bp->uuid} ?? 0);
            $vals[] = (float)$d->total;
            foreach ($vals as $i => $v) $s->setCellValueByColumnAndRow($i + 1, $r, $v);
            $r++;
        }
    ...
    {
        $s = $ss->createSheet();
        $this->excel_title($s, 'DETAIL BATCH', $scope, 'Q');
        $h = ['No', 'Tanggal', 'Plan', 'Kode Batch', 'Varian', 'Mesin', 'Adonan', 'Filkar Box', 'Filkar Kg', 'Sortasi', 'Release', 'Belum Sortir', 'Filkar Rework', 'Filkar Reject', 'Sortasi Rework', 'Sortasi Reject', 'Yield Release'];
        foreach ($h as $i => $v) $s->setCellValueByColumnAndRow($i + 1, 4, $v);
        $this->excel_header($s, 'A4:Q4');
        $r = 5;
        $no = 1;
        foreach ($rows as $d) {
            $vals = [$no++, date('d-m-Y', strtotime($d->tanggal)), $d->plan, $d->kode_batch, $d->varian, $d->nama_mesin, $d->adonan, $d->filkar_box, $d->filkar_kg, $d->sortasi_box, $d->release_box, $d->belum_sortir, $d->bad_filkar_rework_kg, $d->bad_filkar_reject_kg, $d->bad_sortasi_rework_kg, $d->bad_sortasi_reject_kg, $d->yield_release];
            foreach ($vals as $i => $v) $s->setCellValueByColumnAndRow($i + 1, $r, $v);
            $r++;
        }
        foreach (range('A', 'Q') as $c) $s->getColumnDimension($c)->setAutoSize(true);
    }
    private function excel_col($n)
    {
        $s = '';
        while ($n > 0) {
            $n--;
