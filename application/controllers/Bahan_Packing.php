...
use Dompdf\Dompdf;
use Dompdf\Options;
class Bahan_Packing extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('bahan_packing_model');
	...
	{
		$data = array(
			'data' => $this->bahan_packing_model->get_all(),
			'active_nav' => 'bahan_packing'
		);
	
		$this->load->view('partials/head', $data);
		$this->load->view('bahan_packing/bahan_home',$data);
		$this->load->view('partials/footer');
	}

	public function tambah()
	{
		$rules = $this->bahan_packing_model->rules();
		$this->form_validation->set_rules($rules);
		
		if ($this->form_validation->run() === TRUE) {
        $today = date('Y-m-d');
        $tgl = date('Y-m-d', strtotime($today));
			$insert = $this->bahan_packing_model->insert();
			if ($insert) {
				$this->session->set_flashdata('success_msg', 'Data berhasil di simpan.');
				redirect('bahan_packing/detail_'.$tgl);
			} else {
		...
		$data = array(
			'jenis' => $this->bahan_packing_model->get_item(),
			'urut' => $this->bahan_packing_model->generate_reservasi_number(),
			'active_nav' => 'bahan_packing'
		);

		$this->load->view('partials/head', $data);
		$this->load->view('bahan_packing/bahan_tambah');
		$this->load->view('partials/footer');
	}
	...
	{
		$data = array(
			'data' => $this->bahan_packing_model->get_all_packing($tanggal),
			'active_nav' => 'bahan_packing'
		);
	
		$this->load->view('partials/head', $data);
		$this->load->view('bahan_packing/bahan_detail', $data);
		$this->load->view('partials/footer');
	}

    public function approval_packing($tanggal, $area_uuid)
    {
    $update = $this->bahan_packing_model->approval_packing($tanggal, $area_uuid);
...
{
	$data = array(
		'data' => $this->bahan_packing_model->get_kirim_packing($bahan_uuid),
		'nav' => $this->bahan_packing_model->get_by_uuid($bahan_uuid),
		'active_nav' => 'bahan_packing'
	);

	$this->load->view('partials/head', $data);
	$this->load->view('bahan_packing/bahan_kirim', $data);
	$this->load->view('partials/footer');
}
	...
	{
		$update = $this->bahan_packing_model->update_diterima($uuid);
		if ($update) {
    ...
    public function get_item_packing($uuid)
	{
		$data = $this->bahan_packing_model->get_item_packing($uuid);
	...
	public function form($tanggal)
        {
            ...
            $data = $this->bahan_packing_model->get_form_packing($tanggal);
            ...
            <td style="border-left:0;border-right:0;border-bottom:0;height:28px;">&nbsp;5 dari 5</td> 
            </tr>
            </tbody>
            </table>
            </td>
            ...
            <td style="border:none;"> : Packing</td>
