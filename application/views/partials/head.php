...
<a class="sidebar-brand d-flex align-items-center justify-content-center" href="<?= base_url('home') ?>">
    <div class="sidebar-brand-icon mt-5">
        <img src="<?= base_url('assets/img/Prod1.png'); ?>" 
        alt="Logo" 
...
<li class="nav-item <?= in_array($active_nav,['pm','pm-tpm','pm-history']) ? 'active' : '' ?>">
    ...
    <div id="maintenanceMenu" class="collapse <?= in_array($active_nav,['pm','pm-tpm','pm-history']) ? 'show' : '' ?>">
        <div class="bg-white py-2 collapse-inner rounded">

            <a class="collapse-item <?= $active_nav=='pm-tpm'?'active':'' ?>" href="<?= base_url('pm/tpm') ?>">PENGAJUAN</a>

            <a class="collapse-item <?= $active_nav=='pm'?'active':'' ?>" href="<?= base_url('pm') ?>">MONITORING</a>

            <a class="collapse-item <?= $active_nav=='pm-history'?'active':'' ?>" href="<?= base_url('pm/history') ?>">HISTORY</a>
...
<li class="nav-item <?= in_array($active_nav,['am','am-tpm','am-history']) ? 'active' : '' ?>">
    ...
    <div id="amMenu" class="collapse <?= in_array($active_nav,['am','am-tpm','am-history']) ? 'show' : '' ?>">
        <div class="bg-white py-2 collapse-inner rounded">

            <a class="collapse-item <?= $active_nav=='am-tpm'?'active':'' ?>" href="<?= base_url('am/tpm') ?>">PLANNING</a>

            <a class="collapse-item <?= $active_nav=='am'?'active':'' ?>" href="<?= base_url('am') ?>">TASK</a>

            <a class="collapse-item <?= $active_nav=='am-history'?'active':'' ?>" href="<?= base_url('am/history') ?>">HISTORY</a>
        </div>
    </div>
</li>
...
<?php if(is_admin() || is_produksi() || is_engineering() || is_warehouse()){ ?>
    <li class="nav-item <?= in_array($active_nav,['tpm-part','monitor','histori-part']) ? 'active' : '' ?>">
        ...
        <div id="partMenu" class="collapse <?= in_array($active_nav,['tpm-part','monitor','histori-part']) ? 'show' : '' ?>">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item <?= $active_nav=='tpm-part'?'active':'' ?>" href="<?= base_url('monitor/tpm') ?>">PENGAJUAN</a>
                <a class="collapse-item <?= $active_nav=='monitor'?'active':'' ?>" href="<?= base_url('monitor') ?>">MONITORING</a>
                <a class="collapse-item <?= $active_nav=='histori-part'?'active':'' ?>" href="<?= base_url('monitor/history') ?>">HISTORY</a>
            </div>
        </div>
    </li>
...
<li class="nav-item <?= $active_nav == 'pengajuan-part' || $active_nav == 'pengajuan-history' ? 'active' : '' ?>">
    <a class="nav-link collapsed" data-toggle="collapse" data-target="#repairPart">
        <span>REPAIR & NEW PART</span>
    </a>
    <div id="repairPart" class="collapse <?= $active_nav == 'pengajuan-part' || $active_nav == 'pengajuan-history' ? 'show' : '' ?>">
        <div class="bg-white py-2 collapse-inner rounded">
            <a class="collapse-item <?= $active_nav=='pengajuan-part'?'active':'' ?>" href="<?= base_url('partrequest') ?>">PENGAJUAN</a>
            <a class="collapse-item <?= $active_nav=='pengajuan-history'?'active':'' ?>" href="<?= base_url('partrequest/history') ?>">HISTORY</a>
        </div>
    </div>
</li>
<?php } ?>
...
<?php if(is_admin() || is_engineering()){ ?>
    <li class="nav-item <?= $active_nav == 'manual_books' ? 'active' : '' ?>">
        ...
        <div id="manualBook" class="collapse <?= $active_nav == 'manual_books' ? 'show' : '' ?>">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item <?= $active_nav=='manual_books'?'active':'' ?>" href="<?= base_url('manual_books') ?>">MANUAL BOOKS</a>
            </div>
        </div>
    </li>
<?php } ?>
...
<li class="nav-item <?= in_array($active_nav,['formcounter', 'zanasi','cekmesin','sanitasi-data','rework','pergantian_varian','pengenceran']) ? 'active' : '' ?>">
    <a class="nav-link collapsed" data-toggle="collapse" data-target="#formMenu">
        <span>FORM</span>
    </a>
    <div id="formMenu" class="collapse <?= in_array($active_nav,['formcounter', 'zanasi','cekmesin','sanitasi-data','rework','pergantian_varian','pengenceran']) ? 'show' : '' ?>">
        <div class="bg-white py-2 collapse-inner rounded">
            <a class="collapse-item <?= $active_nav=='counter'?'active':'' ?>" href="<?= base_url('counter/') ?>">counter</a>
            ...
            <a class="collapse-item <?= $active_nav=='rework'?'active':'' ?>" href="<?= base_url('mpusage/') ?>">Formula & Rework</a>
...
<?php if($type==1 || $type==2){ ?>
    ...
    <li class="nav-item <?= in_array($active_nav,[
        ...
        class="collapse <?= in_array($active_nav,[
            'area','mesin', 'bahan', 'sparepart','am-data','kegiatan-am','masterspeed',
            'gmp-area', 'gmp-data','m_kondisi','varian','m_tindakan'
            ]) ? 'show':'';?>">
            <div class="bg-white py-2 collapse-inner rounded">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown no-arrow">
                    ...
                    <img src="<?= base_url('assets/img/Oooo.jpeg'); ?>"
                    alt="Profile"
                ...
                <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown" style="min-width: 260px;">
                    ...
                    <div class="dropdown-user-header">
                        ...
                        <div class="meta">
                            <?= $this->session->userdata('username'); ?> • <?= $this->session->userdata('departemen'); ?>
                        </div>
                    </div>
