...
(function () {
    ...
    async function renderBadpro() {
        ...
        const jp=journeyPlan.map(x=>`<tr><td>${esc(x.tanggal_plan)}</td><td>${esc(x.plan)}</td><td>${esc(x.varian||'-')}</td><td>${esc(x.jumlah_batch)}</td><td>${num(x.total_kg)}</td></tr>`).join('');
        const jb=journeyBatch.map(x=>`<tr><td>${esc(x.tanggal_produksi)}</td><td>${esc(x.plan)}</td><td>${esc(x.varian||'-')}</td><td>${esc(x.kode_batch)}</td><td>${num(x.total_kg)}</td></tr>`).join('');
