<div class="container-fluid">\
	<h3 class="h3 mb-2 text-gray-800">Detail Bad Produk SMFG </h3>
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="<?= base_url('view/smfg') ?>"><i class="fas fa-arrow-left mr-2"></i> Data Bad Produk SMFG</a></li>
	...
	<div class="card shadow mb-4">
			...
			<table class="table table-bordered mb-4">
				...
				foreach ($data as $value) {
					...
					<tr>
						...
						<td><?= $value->jumlah ;?></td>
						<td>
							<button class="btn btn-md btn-warning btn-editSmfg btn-block"
							data-uuid="<?= $value->uuid; ?>"
							data-badpro="<?= $value->badpro_uuid; ?>"
							data-jumlah="<?= $value->jumlah; ?>"
							data-toggle="modal"
							data-target="#editSmfg">
							Edit
						</button>
						<!-- Modal Edit -->
						<div class="modal fade" id="editSmfg" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<form action="<?= base_url('view/edit_smfg'); ?>" method="post">
									<div class="modal-content">
										...
										<div class="modal-body">
											<input type="hidden" name="uuid" id="editSmfgUuid">
											<div class="mb-3">
												<label for="editBadpro" class="form-label">Bad Produk</label>
												<select id="editBadproSmfg" name="badpro" class="form-control" required>
											...
											<div class="mb-3">
												...
												<input type="number" class="form-control" id="editJumlahSmfg" name="jumlah" step="0.001" required>
											</div>
										</div>
		...
		<a href="<?= base_url('view/smfg') ?>" class="btn btn-md btn-primary mt-3">
...
<script>
	$(document).on('click', '.btn-editSmfg', function () {
		let uuid = $(this).data('uuid');
  let badpro = $(this).data('badpro'); // sekarang isinya UUID, bukan nama
  let jumlah = $(this).data('jumlah');

  $('#editSmfgUuid').val(uuid);
  $('#editBadproSmfg').val(badpro); // pasti cocok dengan value di option
  $('#editJumlahSmfg').val(jumlah);
});
		...
		$(document).on('click', '.btn-hapus', function(e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.get('<?= base_url('view/hapus_smfg/'); ?>' + data_uuid, function(res) {
						try {
