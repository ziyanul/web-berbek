<div class="container-fluid">\
	<h3 class="h3 mb-2 text-gray-800">Detail Reject SMFG per Mesin</h3>
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="<?= base_url('view/smfgmsn') ?>"><i class="fas fa-arrow-left mr-2"></i> Data Reject SMFG per Mesin</a></li>
	...
	<div class="card shadow mb-4">
				...
				<tbody>
					...
					foreach ($data as $value) {
						...
						<tr>
							...
							<td><?= $value->berat ;?></td>
							<td>
								<button class="btn btn-md btn-warning btn-editSmfgmsn btn-block"
								data-uuid="<?= $value->uuid; ?>"
								data-badpro="<?= $value->badpro_uuid; ?>"
								data-jumlah="<?= $value->berat; ?>"
								data-toggle="modal"
								data-target="#editSmfgmsn"><i class="fa fa-edit mr-2"></i>
								Edit
							</button>
							<!-- Modal Edit -->
							<div class="modal fade" id="editSmfgmsn" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
								<div class="modal-dialog">
									<form action="<?= base_url('view/edit_smfgmsn'); ?>" method="post">
										<div class="modal-content">
											...
											<div class="modal-body">
												<input type="hidden" name="uuid" id="editSmfgmsnUuid">
												<div class="mb-3">
													<label for="editBadpro" class="form-label">Bad Produk</label>
													<select id="editBadproSmfgmsn" name="badpro" class="form-control" required>
												...
												<div class="mb-3">
													...
													<input type="number" class="form-control" id="editJumlahSmfgmsn" name="jumlah" step="0.001" required>
												</div>
											</div>
		...
		<a href="<?= base_url('view/smfgmsn') ?>" class="btn btn-md btn-danger mt-3">
...
<script>
	$(document).on('click', '.btn-editSmfgmsn', function () {
		let uuid = $(this).data('uuid');
  let badpro = $(this).data('badpro'); // sekarang isinya UUID, bukan nama
  let jumlah = $(this).data('jumlah');

  $('#editSmfgmsnUuid').val(uuid);
  $('#editBadproSmfgmsn').val(badpro); // pasti cocok dengan value di option
  $('#editJumlahSmfgmsn').val(jumlah);
});
		...
		$(document).on('click', '.btn-hapus', function(e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.get('<?= base_url('view/hapus_smfgmsn/'); ?>' + data_uuid, function(res) {
						try {
